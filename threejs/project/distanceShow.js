...
function init(model) {
  ...
  let r = 2000;
  let r2 = 1000;
  let x1 = centerModel(model, new THREE.Vector3(xCenter+xDis / 2, yCenter, zDis + r));
  let x2 = centerModel(model, new THREE.Vector3(xCenter-(xDis / 2), yCenter, zDis + r));
  let x3 = centerModel(model, new THREE.Vector3(xCenter+xDis / 2, yCenter, zDis + r + r2));
  let x4 = centerModel(
    model,
    new THREE.Vector3(xCenter-(xDis / 2), yCenter, zDis + r + r2)
  );
  drawLine(xGroup, x1, x2, x3, x4, String(xDis.toFixed(2)) + "m", "x", model);

  let y1 = centerModel(model, new THREE.Vector3(xDis / 2 + r, yDis, -r2));
  let y2 = centerModel(
    model,
    new THREE.Vector3(xDis / 2 + r, -0,- r2)
  );
  let y3 = centerModel(model, new THREE.Vector3(xDis / 2 + r, yDis, -r-r2 ));
  let y4 = centerModel(
    model,
    new THREE.Vector3(xDis / 2 + r, -0, -r-r2)
  );
  drawLine(xGroup, y1, y2, y3, y4, String(yDis.toFixed(2)) + "m", "y", model);

  let z1 = centerModel(model, new THREE.Vector3( - r, yCenter, zDis ));
  let z2 = centerModel(model, new THREE.Vector3( - r, yCenter, 0));
  let z3 = centerModel(
    model,
    new THREE.Vector3( - r - r2, yCenter, zDis)
  );
  let z4 = centerModel(
    model,
    new THREE.Vector3( - r - r2, yCenter, 0)
  );
  drawLine(xGroup, z1, z2, z3, z4, String(zDis.toFixed(2)) + "m", "z", model);
  
  let y11 = centerModel(model, new THREE.Vector3(0, 13989.861898909196, zDis));
  let y21 = centerModel(
    model,
    new THREE.Vector3(0, 12020.943636756334,zDis)
  );
  let y31 = centerModel(model, new THREE.Vector3(0, 13989.861898909196, zDis));
  let y41 = centerModel(
    model,
    new THREE.Vector3(0,12020.943636756334, zDis)
  );
